...
export default function App() {
  ...
  const generate = async () => {
    ...
    const prompt = `You are an expert language learning content creator. Return ONLY valid JSON, no markdown, no backticks.
Goal: "${goalText}"
Target Language: ${langObj.name}
Native Language: English
Return this exact JSON structure:
{"vocabulary":[{"word":"...","translation":"...","example":"..."}],"verbs":[{"verb":"...","translation":"...","yo":"...","tu":"...","el":"..."}],"stories":[{"title":"...","tense":"Present|Past|Future","perspective":"First person|Third person","text":"4-6 sentences","translation":"English translation"}],"dialogues":[{"title":"...","exchanges":[{"speaker":"Role","line":"...","translation":"..."}]}],"exercises":[{"type":"fill-in-the-blank","sentence":"sentence with ___","answer":"word","hint":"English hint"},{"type":"multiple-choice","question":"question in English","options":["A","B","C","D"],"correct":0}]}
Generate: 10 vocabulary, 6 verbs, 3 stories (vary tenses), 2 dialogues (7 exchanges each), 8 exercises (mix types). Return ONLY the JSON.`;
    ...
    try {
      const res = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          model: "gpt-4o-mini",
          messages: [
            { role: "system", content: "You are an expert language learning content creator. Always respond with valid JSON only. No markdown, no backticks, no explanation before or after the JSON." },
  ...
  return(
    <div style={{animation:"fadeIn .4s ease",maxWidth:580}}>
      ...
      <div className="card" style={{marginBottom:14}}>
        ...
        <div className="form-group"><label className="form-label">OpenAI API Key</label><input className="form-input" type="password" placeholder="sk-…" value={s.apiKey} onChange={e=>setS(p=>({...p,apiKey:e.target.value}))}/></div>
        <div style={{fontSize:12,color:"var(--gray)",marginTop:-8}}>Used for AI lesson content generation (OpenAI GPT-4o-mini)</div>
      </div>
